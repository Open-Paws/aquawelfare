// AI/ML Module: Welfare Gap Scoring Algorithm
...
function calculateInterventionFeasibility(speciesData) {
  ...
  if (speciesData.researchCitations && speciesData.researchCitations.length > 0) {
    feasibility += 0.15; // more research = more we know about improvements
  }
  if (speciesData.certificationSchemes && speciesData.certificationSchemes.length > 0) {
    feasibility += 0.1; // existing cert infrastructure
  }
  if (speciesData.taxonomicGroup === 'Fish') {
    feasibility += 0.1; // stunning technology exists for fish
  } else if (speciesData.taxonomicGroup === 'Crustacean') {
    feasibility += 0.05; // some solutions emerging (electrical stunning)
  }
  if (speciesData.annualProductionTonnes > 100000) {
    feasibility += 0.1; // scale makes intervention impactful
  }
